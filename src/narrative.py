...
def generate_operator_narrative(
    ...
    """
    # Deterministic fallback classification rule: SoH <= 80% OR RUL <= 50 cycles triggers Replace Soon, slope <= -0.05 triggers Monitor
    if current_soh <= 80.0 or (rul_likely_cycles is not None and rul_likely_cycles <= 50):
        recommended_status = "Replace Soon"
    elif current_soh <= 85.0 or trend_slope <= -0.05:
